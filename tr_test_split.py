...
def tr_test_splitter(field_name):
    ...
    print(len(js))
    js = [{'image_path':x['image_path'][1:], 'rects' : [{'x1':y['x1']*koef,'x2':y['x2']*koef,'y1':y['y1']*koef,'y2':y['y2']*koef} for y in x['rects']]} for x in js if (x['image_path'][7:] in dirlist3) and (len(x['rects']) > 0)]
