...
imgdir_path = 'real_tif3' #'/Users/tigunova/PycharmProjects/untitled1/imgs'
json_dir = 'jsons3'
parts_path = 'parts/'
...
stuffs = ['traveller', 'pcity', 'tripdate', 'cname', 'pretax']
stuffs_dict = dict((x[1],x[0]) for x in enumerate(stuffs))
        ...
        for wrd in words:
            ...
            for key, k in dic.items():
                ...
                if len(decoded)>1 and (distance*1.0/min(len(k),len(decoded))<tresh):
                    ...
                    sqrs += [{'x1':wrd[1]*koeff,'y1':wrd[2]*koeff,'x2':wrd[3]*koeff,'y2':wrd[4]*koeff, 'label':stuffs_dict[key]}]
                    pages += [page]
                    kwords += [k]
                    uncomb = True
                    continue
                ...
                if len(prev_bx) > 0 and prev_bx[1] < wrd[3] and prev_bx[2] < wrd[4] and prev_bx[4] > wrd[2]:
                    ...
                    if len(decoded)>0 and (distance*2.0/(len(k)+len(comb))<tresh) and not uncomb_prev and not uncomb:
                        ...
                        if key.find('street') != -1 and len(sqrs) > 0 and sqrs[-1]['x1'] == prev_bx[1] and sqrs[-1]['y1'] == prev_bx[2]:
                            sqrs = sqrs[:-1]
                        sqrs += [{'x1':prev_bx[1]*koeff,'y1':prev_bx[2]*koeff,'x2':wrd[3]*koeff,'y2':wrd[4]*koeff, 'label':stuffs_dict[key]}]
                        pages += [page]
                        kwords += [k]
                        continue
                        #print(comb + '-' + k)
...
'''
with open('reference_dict.txt', 'w') as fil:
    w = csv.writer(fil, delimiter=',', lineterminator='\n')
    w.writerows(stuffs_dict.items())
with open('reference_dict.txt', 'r') as fil:
    reader = csv.reader(fil)
    refer_dict = {rows[1]:rows[0] for rows in reader}
print(refer_dict)
